...
const initialStepsData = [
    { id: 1, title: 'Startul Călătoriei', icon: Rocket, description: 'Bun venit pe drumul către Approbation! Completează primii pași pentru a înțelege procesul și a porni la drum cu dreptul.', tasks: [ 
        ...
        { id: 102, text: 'Confirmă adresa de e-mail.', completed: false, viewed: false, action: { type: 'email_verification' } },
...
const AppContent = () => {
    ...
    const { canAccessStep, canAccessOrangeNode, hasAIAccess, subscriptionTier, SUBSCRIPTION_TIERS, upgradeSubscription } = useSubscription();
            ...
            <div className="w-full max-w-md mx-auto">
                <header className="text-center mb-6 bg-white/70 backdrop-blur-sm p-4 rounded-xl shadow-md">
                    ...
                    <div className="relative">
                        <div 
                            className="w-full bg-gray-200 rounded-full h-4 mt-4 overflow-hidden border border-gray-300 cursor-pointer"
                        ...
                        {showProgressOptions && (
                            <div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-white border rounded-lg shadow-lg p-3 z-50">
                                <div className="text-sm font-semibold text-gray-700 mb-2">Mod de Progres</div>
                                <div className="space-y-2">
                                    <button
                                        onClick={() => {
                                            setProgressMode('progressive');
                                    ...
                                    >
                                        🎯 Progresiv
                                        <div className="text-xs text-gray-500">Step-uri se deschid când completezi precedentele</div>
                                    </button>
                                    <button
                                        onClick={() => {
                                            setProgressMode('free');
                                            setShowProgressOptions(false);
                                        }}
                                        className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${
                                            progressMode === 'free' 
                                    ...
                                    >
                                        🔓 Liber
                                        <div className="text-xs text-gray-500">Toate step-urile sunt accesibile</div>
                                    </button>
                                </div>
                                <div className="text-xs text-gray-500 mt-2 pt-2 border-t">
                                    💡 Punctele XP se câștigă doar prin completarea task-urilor
                                </div>
                            </div>
                        )}
                    </div>
