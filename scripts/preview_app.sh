...
# Generate a temporary JWT secret for preview
export JWT_SECRET="preview-secret-key-$(date +%s)"
export MONGO_URL="mongodb://localhost:27017/approbmed_preview"
export DB_NAME="approbmed_preview"
export ENVIRONMENT="development"
export ALLOWED_ORIGINS="http://localhost:3000,http://localhost:8000"

# Set dummy API keys for preview (features will be limited)
export GEMINI_API_KEY="dummy-key-for-preview"
export STRIPE_API_KEY="dummy-key-for-preview"

echo "Preview Configuration:"
echo "- API URL: http://localhost:8000"
echo "- Docs URL: http://localhost:8000/docs"
echo "- MongoDB: Local instance (mongodb://localhost:27017)"
echo "- Environment: Development"
echo ""
echo "Note: This is a preview mode with limited features:"
echo "- AI Assistant will return mock responses"
echo "- Payment features are disabled"
echo "- Email features are disabled"
echo ""

# Check if MongoDB is installed
if ! command -v mongod &> /dev/null; then
    echo "WARNING: MongoDB is not installed. Using in-memory mock database."
...
pip install -q fastapi uvicorn motor python-jose passlib python-multipart aiofiles pydantic[email]

# Create a minimal server file for preview if main server has issues
cat > preview_server.py << 'EOF'
...
if python3 -c "import server" 2>/dev/null; then
    ...
    echo "Running preview server with mock data..."
